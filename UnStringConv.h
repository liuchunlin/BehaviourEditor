...
public:
	 ...
	 */
	__inline FTCHARToANSI_Convert(DWORD InCodePage = CP_ACP) :
		CodePage(InCodePage)
	{
	}
	 ...
	 */
	__inline ANSICHAR* Convert(const TCHAR* Source,ANSICHAR* Dest,DWORD Size)
	{
	...
	__inline UINT Length(ANSICHAR* Dest)
	{
#if _MSC_VER
...
public:
	 ...
	 * @param Size the size of the dest buffer in bytes
	 */
	__inline TCHAR* Convert(const ANSICHAR* Source,TCHAR* Dest,DWORD Size)
	{
		// Determine whether we need to allocate memory or not
#if _MSC_VER
	...
	/* return the string length without the null terminator */
	__inline UINT Length(TCHAR* Dest)
	{
#if _MSC_VER
...
public:
    ...
    __inline static void utf8fromcodepoint(DWORD cp, ANSICHAR **_dst, DWORD *_len)
    {
        char *dst = *_dst;
        INT len = *_len;
...
public:
    ...
    // I wrote this function for originally for PhysicsFS. --ryan.
    // !!! FIXME: Maybe this shouldn't be inline...
    __inline static DWORD utf8codepoint(const ANSICHAR **_str)
    {
        const char *str = *_str;
        DWORD retval = 0;
