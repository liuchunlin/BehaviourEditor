...
void CheckRunTime::checkTimeOut(int timeout)
{
	...
	if(ms>=timeout)
	{
		flog_f("time",".log","%s:%d:(%s) cost:%d",m_file,m_line,m_name,ms);
		if(m_context_file1)
		{
			flog_f("time",".log","context1 %s:%d",m_context_file1,m_context_line1);
		}
		if(m_context_file2)
		{
			flog_f("time",".log","context2 %s:%d",m_context_file2,m_context_line2);
		}
	}
}
