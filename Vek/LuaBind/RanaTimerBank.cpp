...
RanaTimerBank::~RanaTimerBank()
{
	multiset<RanaTimer*,CompRanaTimerPoint>::iterator itActive;
	...
	m_activeTimer.clear();
	list<RanaTimer*>::iterator itPaused;
...
bool RanaTimerBank::delTimer(const RanaSpaceHandle& _spaceHandle)
{
	...
	if(!pSpace)
	{
		...
		{
			debug_ranaLog("RanaTimerBank::deltimer : handle[%d] serial error!\n", _spaceHandle.handle);
		}
		return false;
	}
...
void RanaTimerBank::routine(RanaTIME _expiretime)
{
	...
	multiset<RanaTimer*,CompRanaTimerPoint>::iterator timerIt;
...
RanaTimer* RanaTimerBank::getTimer(const RanaSpaceHandle& _spaceHandle, RanaTimerHandle _timerHandle)
{
	RanaSpace* pSpace = RanaSpaceBank::getInstance()->getLuaSpace(_spaceHandle);
	if(!pSpace)
	{
...
void RanaTimerBank::setTimerPeriodTime(const RanaSpaceHandle& _spaceHandle, RanaTimerHandle _timerHandle, RanaTIME _periodTime)
{
...
RanaTIME RanaTimerBank::getTimerPeriodTime(const RanaSpaceHandle& _spaceHandle, RanaTimerHandle _timerHandle)
{
	RanaTimer *timer = getTimer(_spaceHandle, _timerHandle);
	if (timer)
	{
...
void RanaTimerBank::setTimerRepeat(const RanaSpaceHandle& _spaceHandle,RanaTimerHandle _timerHandle, bool _isRepeat)
{
	RanaTimer *timer = getTimer(_spaceHandle, _timerHandle);
	if (timer)
	{
...
bool RanaTimerBank::getTimerRepeat(const RanaSpaceHandle& _spaceHandle, RanaTimerHandle _timerHandle)
{
	RanaTimer *timer = getTimer(_spaceHandle, _timerHandle);
	if (timer)
	{
...
void RanaTimerBank::setTimerOnOff(const RanaSpaceHandle& _spaceHandle, RanaTimerHandle _timerHandle, bool _isOn)
{
	RanaTimer *timer = getTimer(_spaceHandle, _timerHandle);
	if (timer)
	{
		...
		if(isCurOn!=_isOn)
		{
			if(_isOn)
			{
				...
				{
					timer->setOnOff(true);
					timer->setExpireTime(g_Clock->getMilliSeconds() + m_restTime);
					m_pausedTimer.erase(pListIterator->pausedIt);
...
bool RanaTimerBank::getTimerOnOff(const RanaSpaceHandle& _spaceHandle, RanaTimerHandle _timerHandle)
{
	RanaTimer *timer = getTimer(_spaceHandle, _timerHandle);
	if (timer)
	{
...
RanaTIME RanaTimerBank::getTimerRestTime(const RanaSpaceHandle& _spaceHandle, RanaTimerHandle _timerHandle)
{
	RanaTimer *timer = getTimer(_spaceHandle, _timerHandle);
	if (timer)
	{
...
void RanaTimerBank::resetTimer(const RanaSpaceHandle& _spaceHandle, RanaTimerHandle _timerHandle)
{
	RanaTimer *timer = getTimer(_spaceHandle, _timerHandle);
	if (timer)
	{
