...
#ifdef _DEBUG
#define RANA_ASSERT(x)	\
assert(x);
#else
#define RANA_ASSERT(x) x;
#endif

#define RANA_OK 0

// 常用类型
typedef unsigned char		RanaBYTE;
typedef unsigned short int	RanaWORD;
typedef	unsigned int		RanaDWORD;
...
struct RanaVariant	//Rana通用变量
{
	...
	RanaVariant & operator=(double val);

	bool operator==( const RanaVariant& obj) const;
};
...
inline RanaVariant::operator double () const
{
	if(type==VT_REAL)
	{
		return body.dVal;
...
inline RanaVariant & RanaVariant::operator=(double val)
{
	type=VT_REAL;
	body.dVal=val;
	return *this;
...
inline bool RanaVariant::operator==( const RanaVariant& obj) const
{
	if( type != obj.type )
		return false;
	...
	{
	case VT_BOOL:
		return body.bVal == obj.body.bVal;
		break;
	case VT_INTEGER:
		return body.iVal == obj.body.iVal;
		break;
	case VT_STRING:
		return strcmp(body.sVal, obj.body.sVal) == 0;
		break;
	case VT_REAL:
		return body.dVal == obj.body.dVal;
		break;
	case VT_USERDATA:
		return body.vVal == obj.body.vVal;
		break;
	default:
		return false;
	}
...
typedef RanaWORD		RanaEventType;
