...
void RanaCheckType<T>::validateCheck(RanaState L,int index)
{
	...
	{
		throw new RanaInputArgumentException(" is not a table.");
	}
}
...
inline void RanaCheckType<bool>::validateCheck(RanaState L,int index)
{
	...
	{
		throw new RanaInputArgumentException(" is not a boolean.");
	}
}
...
inline void RanaCheckType<char>::validateCheck(RanaState L,int index)
{
	...
	{
		throw new RanaInputArgumentException(" is not a number.");
	}
}
...
RanaInputStack & operator >> (RanaInputStack &s,T &val)
{
	...
	}catch(RanaInputArgumentException *e)
	{
		...
		char buf[20];
		_snprintf_s(buf,sizeof(buf),"arg%d",s.getIndex());
		buf[sizeof(buf)-1]=0;
		e->appendPrefix(buf);
		throw e;
	}
...
void rana_get(RanaState L,int index,const char *name,T &val)
{
	if(!lua_istable(L,index))
	{
		RanaInputArgumentException *e = new RanaInputArgumentException(" is not a table.");
		e->appendPrefix(name);
		throw e;
	}
...
inline void rana_get(RanaState L,int index,RanaVariant& val)
{
	...
	default:
		val.clear();
		throw new RanaInputArgumentException(" is not a simple value.");
		break;
	}
}
