...
{//协议相关类型定义
	...
	struct Rana_Event
	{
		struct Argument	//事件参数
		{
			...
			BYTE	type;	//ArgumentType
			union BODY
			{
				BYTE										bVal;	//布尔型值 (type==AT_BOOL)
				int											iVal;	//整型值 (type==AT_INTEGER)
				Zoic::StrCharArray<RANA_STRING_PARAM_LEN>	sVal;	//字符串值 (type==AT_STRING)
			}		body;	
		};
		Rana_EventType			type;	//事件类型code
...
{//协议定义
	...
	static const WORD TS_LOGIN_REQ=1;
	...
	static const WORD ST_LOGIN_ACK=2;
	...
	struct P_ST_GetEventDetail_Ack
	{
		...
		/*
			...
			ERROR_RANA_EVENT
		*/
		Rana_Event	event;			//事件的类型和参数 (code==ERROR_OK)
		WORD		triggerHandle;	//对应触发器句柄 (code==ERROR_OK)
	};

	//保存Rana空间请求 RanaClient->RanaService
	static const WORD TS_SAVESPACE_REQ=54;
	struct P_TS_SaveSpace_Req
	{
		WORD	space;	//命名空间名柄
	};

	//保存Rana空间响应 RanaService->RanaClient
	static const WORD ST_SAVESPACE_ACK=55;
	struct P_ST_SaveSpace_Ack
	{
		ErrorType	code;	//返回码
		/*
			ERROR_OK
...
{//封包函数
	...
	inline Zoic::Message & operator << ( Zoic::Message &m,const Rana_Event::Argument & p )
	{
		m << p.type;
		switch(p.type)
		{
	...
	inline Zoic::Message & operator >> ( Zoic::Message &m,Rana_Event::Argument & p )
	{
		m >> p.type;
		switch(p.type)
		{
	...
	inline Zoic::Message & operator << ( Zoic::Message &m,const Rana_Event & p )
	{
		m << p.type;
		m << p.args;
		return m;
	...
	inline Zoic::Message & operator >> ( Zoic::Message &m,Rana_Event & p )
	{
		m >> p.type;
		m >> p.args;
		return m;
	...
	inline Zoic::Message & operator << ( Zoic::Message &m,const P_TS_SaveSpace_Req & p )
	{
		m << p.space;
		return m;
	}

	inline Zoic::Message & operator >> ( Zoic::Message &m,P_TS_SaveSpace_Req & p )
	{
		m >> p.space;
		return m;
	}

	inline Zoic::Message & operator << ( Zoic::Message &m,const P_ST_SaveSpace_Ack & p )
	{
		m << p.code;
		return m;
	}

	inline Zoic::Message & operator >> ( Zoic::Message &m,P_ST_SaveSpace_Ack & p )
	{
		m >> p.code;
		return m;
	}
