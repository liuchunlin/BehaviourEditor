...
static void pushListEventBody(std::vector<P_ST_ListEventBody_Ntf::info> & vec,RanaEvent* const & pRanaEvent)
{
	...
	tempInfo.triggerHandle = pRanaEvent->getTriggerHandle();
...
SESSION_COMMAND(RanaClient,TS_RELOADSPACE_REQ)
{
	...
	if( self->checkAccess(RSAM_SPACE_RELOAD) )
	{
		...
		strFilePath += ".lua";
		RanaSpace* pSpace = RanaSpaceBank::getInstance()->forceFindSpaceByName( (const char*)pkt.name );
		if( pSpace && RanaSpaceBank::getInstance()->reloadSpace( pSpace->getHandle(), strFilePath.c_str() ) == 0 )
		{
			pktSend.code = ERROR_OK;
		}
		else
...
SESSION_COMMAND(RanaClient,TS_SUSPENDSPACE_REQ)
{
	...
	if( self->checkAccess(RSAM_TRIGGER_MANTAIN) )
	{
		...
		if(pRanaSpace)
		{
			...
			if(pRanaTriggerBank)
			{
				pRanaTriggerBank->setAllTriggerOnOff(false);
				pktSend.code = ERROR_OK;
			}
			else
			{
...
SESSION_COMMAND(RanaClient,TS_RESUMESPACE_REQ)
{
	...
	if( self->checkAccess(RSAM_TRIGGER_MANTAIN) )
	{
		...
		if(pRanaSpace)
		{
			...
			if(pRanaTriggerBank)
			{
				pRanaTriggerBank->setAllTriggerOnOff(true);
				pktSend.code = ERROR_OK;
			}
			else
			{
...
SESSION_COMMAND(RanaClient,TS_SUSPENDTRIGGER_REQ)
{
	...
	if( self->checkAccess(RSAM_TRIGGER_MANTAIN) )
	{
		...
		if(pRanaSpace)
		{
			...
			if(pRanaTriggerBank)
			{
				if( !pRanaTriggerBank->setTriggerOnOff(pkt.handle,false) )
				{
					pktSend.code = ERROR_OK;
				}
				else
...
SESSION_COMMAND(RanaClient,TS_RESUMETRIGGER_REQ)
{
	...
	if( self->checkAccess(RSAM_TRIGGER_MANTAIN) )
	{
		...
		if(pRanaSpace)
		{
			...
			if(pRanaTriggerBank)
			{
				if( !pRanaTriggerBank->setTriggerOnOff(pkt.handle,true) )
				{
					pktSend.code = ERROR_OK;
				}
				else
	...
	if( self->checkAccess(RSAM_QUERY) )
	{
		...
		if(pRanaSpace)
		{
			...
			if(pRanaTriggerBank && pRanaEventBank)
			{
				...
				std::set<RanaEvent*> events;
				pRanaEventBank->listEventByTrigger( pkt.handle, events );
				std::set<RanaEvent*>::const_iterator it;
				for(it = events.begin();it != events.end(); it++)
				{
					pktSend.eventhandles.push_back((*it)->getHandle());
				}
				pktSend.code = ERROR_OK;
			}
			else
...
SESSION_COMMAND(RanaClient,TS_LISTEVENT_REQ)
{
	...
	pRanaEventBank->listEvents(output);
	pktSend.total = (WORD)output.size();
	pktSend.code = ERROR_OK;
...
SESSION_COMMAND(RanaClient,TS_SUSPENDEVENT_REQ)
{
	...
	if( self->checkAccess(RSAM_EVENT_MANTAIN) )
	{
		...
		if(pRanaSpace)
		{
			...
			if(pRanaEventBank)
			{
				if( !pRanaEventBank->setEventOnOff(pkt.handle,false) )
				{
					pktSend.code = ERROR_OK;
				}
				else
...
SESSION_COMMAND(RanaClient,TS_RESUMEEVENT_REQ)
{
	...
	if( self->checkAccess(RSAM_EVENT_MANTAIN) )
	{
		...
		if(pRanaSpace)
		{
			...
			if(pRanaEventBank)
			{
				if( !pRanaEventBank->setEventOnOff(pkt.handle,true) )
				{
					pktSend.code = ERROR_OK;
				}
				else
...
SESSION_COMMAND(RanaClient,TS_GETEVENTDETAIL_REQ)
{
	...
	if( self->checkAccess(RSAM_QUERY) )
	{
		...
		if(pRanaSpace)
		{
			...
			if(pRanaEventBank)
			{
				RanaEvent* pRanaEvent = pRanaEventBank->getEvent(pkt.handle);
				if(pRanaEvent)
				{
					pktSend.triggerHandle = pRanaEvent->getTriggerHandle();
					const char* pEventTypeCode = g_RanaService->getStrategy()->GetEventCodeFromType( pRanaEvent->getType() );
					if(pEventTypeCode && strcmp(pEventTypeCode,"") )
					{
						pktSend.event.type.useCode = 1;
						pktSend.event.type.code = pEventTypeCode;
					}
					else
					{
						pktSend.event.type.useCode = 0;
						pktSend.event.type.id = pRanaEvent->getType();
					}
...
SESSION_COMMAND(RanaClient,TS_SAVESPACE_REQ)
{
	RanaClient * self=static_cast<RanaClient *>(pSession);
	P_TS_SaveSpace_Req pkt;
	msg >> pkt;
	self->saveSpace(pkt.space);
